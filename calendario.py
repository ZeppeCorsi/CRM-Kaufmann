...
def carregar_aba(nome_aba):
    try:
        ...
        # NORMALIZAÇÃO: Limpa espaços e garante que a coluna de hora seja texto
        # Ajustado para aceitar tanto 'HORA' quanto 'HORARIO'
        df.columns = [str(c).strip().upper() for c in df.columns]
        
        col_hora = "HORARIO" if "HORARIO" in df.columns else "HORA"
        ...
        if nome_aba == "Agendamentos" and "DATA" in df.columns:
            ...
            df = df.sort_values(by=["DATA_DT", col_hora])
        return df
        ...
        with cols[i]:
            ...
            if not df_ag.empty and 'DATA_DT' in df_ag.columns:
                ...
                for idx, v in visitas_dia.iterrows():
                    ...
                    # BUSCA SEGURA PELA HORA OU HORARIO
                    hora_val = v.get('HORARIO', v.get('HORA', '')).strip()
                    hora = hora_val if hora_val and hora_val.lower() != "nan" else "--:--"
